...
async function send (sendInfo) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      user: '1500529741@qq.com', // generated ethereal user
  ...
  const info = await transporter.sendMail({
    from: '"认证邮件" <1500529741@qq.com>', // sender address
    to: sendInfo.email, // list of receivers
    subject:
      sendInfo.user !== '' && sendInfo.type !== 'email'
        ? `你好小伙伴，${sendInfo.user}！考研社区${sendInfo.type === 'reset' ? '重置密码链接！' : '注册码！'}`
        : '考研社区 确认修改邮件链接', // Subject line
    text: `您在 考研社区 中注册，您的邀请码是${
      sendInfo.code
